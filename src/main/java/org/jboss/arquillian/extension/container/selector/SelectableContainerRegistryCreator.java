 ...
 */
package org.jboss.arquillian.extension.container.selector;
 ...
 *
 */
public class SelectableContainerRegistryCreator extends ContainerRegistryCreator {
    ...
    public void createRegistry(@Observes ArquillianDescriptor event) {
        ...
        String activeConfiguration = getActivatedConfiguration(event);
    ...
    private String getActivatedConfiguration(ArquillianDescriptor event) {

        for (ExtensionDef extension : event.getExtensions()) {
            if ("container-selector".equals(extension.getExtensionName())) {
                for (Map.Entry<String, String> entry : extension.getExtensionProperties().entrySet()) {
        ...
        InputStream arquillianLaunchStream = SecurityActions.getThreadContextClassLoader().getResourceAsStream(
